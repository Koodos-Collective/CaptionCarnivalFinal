...
function createChannels(message, server, eventName, imageURL, channelServerData, serverData, eventNum, database, serverID) {
    ...
    message.guild.channels.create(`🤡-${eventName}`, {
            ...
            let introEmbed = new Discord.MessageEmbed()
                ...
                .addFields({
                    ...
                    name: 'Message the bot in this channel to add captions to the meme using \`++caption <meme_caption>\`',
            ...
            if (channelServerData != undefined && channelServerData.eventParticipantRoleID != 0) {
                channel.send(`<@&${channelServerData.eventParticipantRoleID}> There's a new Circus in the server!`);
            }
            ...
            database.collection(`servers/${serverID}/events`).doc("" + eventNum).update({
...
module.exports = {
    ...
    execute(client, message, author, args, database) {
        ...
        if (args.length != 2) {
            message.reply('you need to specify a name and an image for the Caption Circus! Use the following format');
            return message.channel.send("Use the following format: `++new <caption_Circus_name> <base_image_url>`");
        ...
        serverData.get().then((docSnapshot) => {
            if (!docSnapshot.exists) {
                ...
                createChannels(message, server, eventName, imageURL, docSnapshot.data(), serverData, 1, database, serverID);
                ...
                if (docSnapshot.data().eventChannelNames.includes(eventName)) {
                    return message.channel.send(`❌ There is already a Circus with that name for this server! Try a different name.`);
                } else {
                    if (docSnapshot.data().totalNumberOfActiveEvents != 100) {
                        let currentEventNumber = parseInt(docSnapshot.data().totalNumberOfEvents);
                        database.collection(`servers/${serverID}/events`).doc(`${(currentEventNumber + 1)}`).set({
                            channelID: 0,
                            name: eventName,
                            imageURL: imageURL,
                            submissionNum: 0,
                        ...
                        createChannels(message, server, eventName, imageURL, docSnapshot.data(), serverData, currentEventNumber + 1, database, serverID);
                        ...
                        return message.channel.send(`❌ You've reached the limit for active ongoing Caption Circuss!`);
                    }
                }
            }
        });
